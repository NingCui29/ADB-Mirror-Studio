...
> 当前版本：`V1.7.0`。个人、组织和企业均可在合法授权的设备上免费使用全部功能。
...
- “概览”约每 2 秒显示当前设备的整机 CPU/GPU/内存占用、最近 30 秒平均占用和 CPU/GPU 温度；设备未提供可读 GPU 或温度节点时显示“不可用”
...
1. “概览”提供设备详情、截图、连接管理，以及当前设备的 CPU/GPU/内存占用和 CPU/GPU 温度。CPU 温度取最高的明确 CPU 热区；采样规则见 [设备性能监测说明](https://github.com/NingCui29/ADB-Mirror-Studio/blob/master/docs/device-performance-monitoring.md)。
...
commercial\artifacts\release\
  AdbMirrorStudio-V1.7.0-win-x64.zip
```
...
commercial\artifacts\installer\
  ADB-Mirror-Studio-Setup-V1.7.0-win-x64.exe
```
